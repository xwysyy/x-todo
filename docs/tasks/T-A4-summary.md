...
X-TODO 从固定浅色纸张视觉重构为完整主题系统。渲染层不再消费全局颜色常量，主题化 surface（主窗口、折叠胶囊、菜单、确认弹窗、行内编辑框、任务栏状态条）统一读取解析后的 `ThemeVisual` 快照。主题来源包括 8 套内置主题、`%APPDATA%\x-todo\themes\` 下的自定义 `.xtheme` 文件、以及系统明暗模式与高对比跟随。托盘保持固定应用图标。

## 实现范围（7 个阶段）

1. 主题数据模型 + 内置主题 catalog：重写 `Theme.h`，新增 `Theme.cpp` / `ThemeCatalog.{h,cpp}`，vendoring `nlohmann/json` v3.11.3 单头。
2. `Theme::ResolveTheme` + `MainWindow` 持有 `theme_` 快照，调整 `Create()` 初始化顺序使快照在首帧渲染前就绪。
3. 渲染迁移：`MainWindowView` 主窗口与胶囊、确认弹窗、popup 菜单、`WM_CTLCOLOREDIT`、任务栏状态条全部切到主题快照。
...
- **托盘图标固定应用身份 + 生命周期**：`CreateTrayIconHandle` 直接从 `app.ico` 加载 owned `HICON`，`RefreshTrayIcon` 处理 `NIM_MODIFY`、旧 `HICON` 销毁、新 `HICON` 失败回滚。
...
- 托盘固定应用图标、`NIM_MODIFY` 更新、Explorer 重启重建。
...
主窗口、胶囊、菜单、确认弹窗、行内编辑框的主题化与系统明暗 / 高对比跟随均已落地。托盘保持固定应用图标。仍需 Windows 实机视觉核验（见上文「Windows 实机待验证」）。
