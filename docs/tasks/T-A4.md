...
> **版本 v1.1**：已纳入 GPT 5.5 Pro R1 复核的 must-fix 修订。v1 的总体方向保留，修订重点是收敛主题字段语义、修正 Store 精确解析、补齐原生 EDIT 与高对比边界、明确托盘固定应用图标边界，并避免把本地主题文件做成插件系统。
...
- **目标**：把 X-TODO 从固定浅色纸张视觉，重构为完整主题系统。主题系统覆盖主窗口、折叠胶囊、菜单、确认弹窗、行内编辑框和后续任务栏状态条。托盘保持固定应用图标。主题来源包括内置主题、自定义主题文件、系统明暗模式跟随。
...
直接新增几套颜色会留下多个不一致表面：主窗口、弹窗、菜单、编辑框和胶囊有不同绘制技术；后续任务栏模式还有独立绘制入口。全面皮肤系统需要先定义主题数据模型和消费边界，再迁移所有颜色读取点。托盘图标是应用身份，不进入主题消费边界。
...
5. **深色主题为一等功能**：深色主题必须覆盖编辑框、确认弹窗、菜单、胶囊和系统明暗模式跟随，不能只换主窗口背景。
...
9. **托盘图标固定应用身份**：托盘图标直接加载 `resources/app.ico`。主题切换、任务数量变化和自定义主题文件都不得改变托盘图标外观。
...
8. 调用 `RefreshTrayIcon()`，保持托盘使用固定应用图标。
...
`CreateTrayIconHandle()` 固定加载 `resources/app.ico`：

- 使用 `SM_CXSMICON` / `SM_CYSMICON` 请求系统托盘尺寸。
- 系统尺寸不可用时回退到 32 像素。
- 返回 owned `HICON`，交给 `nid_.hIcon` 生命周期管理。
- 不按主题、任务数量或自定义主题文件绘制动态图标。

Explorer 重启后 `AddTrayIcon()` 会重建图标，必须仍然使用 `resources/app.ico`。
...
- `resources/app.ico` 加载失败时记录 runtime notice。
- icon 遵守同一所有权规则：成功交给 `nid_` 后由 `RemoveTrayIcon()` 或下次替换销毁；失败则立即 `DestroyIcon()`。
- Explorer 重启后的 `AddTrayIcon()` 必须使用固定应用图标。
...
- 对主窗口、胶囊、主题管理窗口、任务栏状态条同步刷新。托盘保持固定应用图标。
...
10. 增加 `RefreshTrayIcon()`，保持托盘使用固定应用图标。
...
- **[AC-9]** `CreateTrayIconHandle()` 直接加载 `resources/app.ico`，且失败时有错误记录。
...
1. 切换主题后托盘图标仍为固定应用图标。
2. Explorer 重启后托盘图标恢复为固定应用图标。
3. `resources/app.ico` 加载失败时记录运行时错误。
...
5. 托盘固定应用图标的生命周期规则是否足够，是否还有 `HICON` 泄漏边界。
